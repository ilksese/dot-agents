...
function copyEntry(srcPath: string, destPath: string, dryRun: boolean): void {
  ...
  if (stat.isDirectory()) {
    fs.cpSync(srcPath, destPath, {
      recursive: true,
      force: true,
      filter: (src) => !EXCLUDED_FILES.has(path.basename(src)),
    })
  } else {
...
function listSelectedResources(selection: ListSelection = {}): void {
  ...
  const groups = RESOURCE_TYPES.filter((type) => listAll || (selection[type.option]?.length ?? 0) > 0).map((type) => ({
    label: type.dir,
...
function discoverSelectedPlugins(requested: string[] | undefined): { matched: string[]; missing: string[] } {
  ...
  const pluginsWithoutIndex = matched.filter(
    (name) => !plugins.some((plugin) => path.dirname(plugin) === path.resolve(pluginsDir, name)),
  )
...
program
  ...
  .action((options) => {
    const baseDir = options.target ? path.resolve(options.target) : path.join(os.homedir(), ".config", "opencode")
...
program
  ...
  .option(
    "-p, --plugin [glob]",
    "Plugin glob to list; repeatable; omit glob for every plugin",
    collectOptionalOption,
    [],
  )
  ...
  .option(
    "-c, --command [glob]",
    "Command glob to list; repeatable; omit glob for every command",
    collectOptionalOption,
    [],
  )
  .action((options) => {
