...
- 登录要求可选。用户未明确说明无需登录时，默认需要登录。
- 登录要求可从自然语言中推断：
  - 需要登录、要登录、登录后、已登录态、带登录态 -> 需要登录
  - 不需要登录、无需登录、游客态、未登录态、不登录 -> 不需要登录
...
- 默认需要登录，除非用户明确说明不需要登录、无需登录、游客态、未登录态或不登录。
- 需要登录时，先打开目标环境页面并让用户手动完成登录。
- 提示用户：请在浏览器中手动登录，登录完成后回复 `已登录`。
- 在用户回复 `已登录` 前，不要继续执行后续定位流程。
- 用户回复 `已登录` 后，继续进行问题复现、根因定位和修复方案制定。
- 不需要登录时，直接继续后续定位流程。
